    ...
    public class SearchForm : Form
    {
            ...
            for (int i = 0; i < workbook.NumberOfSheets; i++)
            {
                ...
                for (int rowIndex = sheet.FirstRowNum; rowIndex <= sheet.LastRowNum; rowIndex++)
                {
                    ...
                    for (int colIndex = row.FirstCellNum; colIndex < row.LastCellNum; colIndex++)
                    {
                        ...
                        {
                            string result = String.Format("{0} - セル[{1},{2}]：{3}", sheet.SheetName, rowIndex + 1, colIndex + 1, cellValue);
                            outputForm.AddResult(System.IO.Path.GetFileName(filePath), File.GetLastWriteTime(filePath), filePath, result, "Excelファイルgrep検索結果");
                        }
                    }
                }
            }
        ...
        private void SearchWordFile(string filePath, string searchWord, Regex regex, OutputForm outputForm)
        {
            using (var document = DocX.Load(filePath))
            {
                ...
                {
                    string result = String.Format("ファイル内容：\r\n{0}", text);
                    outputForm.AddResult(System.IO.Path.GetFileName(filePath), File.GetLastWriteTime(filePath), filePath, result, "Wordファイルgrep検索結果");
                }
            }
        }
        ...
        private void SearchPdfFile(string filePath, string searchWord, Regex regex, OutputForm outputForm)
        {
            using (PdfReader reader = new PdfReader(filePath))
            {
                for (int i = 1; i <= reader.NumberOfPages; i++)
                {
                    ...
                    {
                        string result = String.Format("ページ{0}：\r\n{1}", i, pageText);
                        outputForm.AddResult(System.IO.Path.GetFileName(filePath), File.GetLastWriteTime(filePath), filePath, result, "PDFファイルgrep検索結果");
                    }
                }
            }
        }
